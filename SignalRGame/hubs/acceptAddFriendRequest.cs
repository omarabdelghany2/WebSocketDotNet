...
namespace SignalRGame.Hubs
{
    public partial class GameHub
    {
        public async Task acceptAddFriend(acceptAddFriendRequest request)
        {
            ...
            if (userId!=0)
            {
                await Clients.Caller.SendAsync("acceptAddFriendRequest", new{friendId = friendId, error=true,errorMessage="Error retrieving userId; something went wrong with the Token."});
            ...
            if (LoginRoomMapping.TryGetValue(friendId.ToString(), out var loginRoomConnectionId))
            {
                ...
                await Clients.Group(loginRoomConnectionId).SendAsync("acceptedFriendRequest", new{userId=userId, profileName = profile?.profileName,score=score});
            }
